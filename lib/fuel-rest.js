	...
	_processRequest(options, callback) {
		...
		this.AuthClient.getAccessToken(requestOptions)
			...
			.then(tokenInfo => {
				...
				const accessToken = tokenInfo.accessToken ? tokenInfo.accessToken : tokenInfo.access_token
				...
				options.headers = Object.assign({}, this.defaultHeaders, options.headers, {
					...
					Authorization: (headers && headers.Authorization) || 'Bearer ' + accessToken
				});
				...
				consolidatedOpts.accessToken = accessToken;
